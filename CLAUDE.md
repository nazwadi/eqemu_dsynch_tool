...
├── conditions.go    # Conditions tab: CompareZoneConditions — read-only visibility for
│                     spawn_conditions/spawn_condition_values/spawn_events (added 2026-07-25,
│                     see Sync Design)
...
└── frontend/
    └── src/
        ...
        │   ├── useSpawnConditions.js    # Conditions tab (added 2026-07-25) — read-only, no
        │   │                             selection/sync state at all, unlike every other domain hook
        │   ├── useModalFocusTrap.js     # Shared focus-on-open + Escape-to-close behavior,
        │   │                             used by all 9 modal/drawer components below
        │   └── useListArrowKeyNav.js    # Shared Up/Down row navigation for diff lists (added
        │                                 2026-07-25), used by NpcsTab/SpawnsTab/GridsTab (List
        │                                 view)/SpawngroupsTab
        ...
        └── components/     # Presentational components, one per modal/drawer/tab/panel
            ...
            │   GridMapHelpDrawer.jsx, LootHelpDrawer.jsx, ConditionsHelpDrawer.jsx  # one per tab
            │   that has inline help — see the "Inline help drawers extended" Sync Design bullet
            │   for why Grids List and TODO don't have one
            ...
            │   LootTab.jsx, ConditionsTab.jsx  # ConditionsTab is read-only (see conditions.go) —
            │   no selection, no detail panel, no sync action anywhere in it
            ├── ZoneMapView.jsx  # Brewall's Maps background + every zone grid overlaid, used
            │                     by GridsTab's Map view (added 2026-07-24)
            ├── IconLegend.jsx  # Always-visible row-icon legend (added 2026-07-25), the same
            │                     pattern ZoneMapView's Grids Map legend already used, generalized
            │                     — used by NpcsTab/SpawngroupsTab so far
...
// SpawnCondition is one spawn_conditions row (conditions.go, added 2026-07-25). PRIMARY KEY
// (zone, id), id NOT auto-increment (confirmed via SHOW CREATE TABLE) — zone-scoped and stable,
// the same trust category as grid.id, unlike spawngroup.id/spawn2.id/lootdrop.id. That's what
// makes a real cross-database diff possible here.
type SpawnCondition struct {
    Id     int64
    Fields map[string]interface{}  // spawn_conditions columns, minus zone/id
}

// SpawnConditionDiffRow mirrors GridDiffRow's match-by-Id shape, simpler — no child/entries table
// of its own, so one Status is enough (no FieldsDiffer/EntriesDiffer split needed).
...
// "condition_values", not "conditions_values" (the user's own first guess, corrected by pulling
// the actual schema before writing any code — see the Sync Design entry for the full story). No
// PRIMARY KEY, unique on (id, instance_id, zone). instance_id is assigned per running zone
// instance by the server (EQEmu's instanced-zone feature) — meaningless across two independent
// server processes, which is what makes this live/runtime state (a boss up/down flag, a
// quest-stage gate a script flips) rather than authored content. Never matched/diffed against the
// other database's rows for exactly that reason.
type SpawnConditionValue struct {
    Id, InstanceId, Value int64  // Value column is nullable — NULL collapses to 0 via toInt64, same convention used elsewhere
}

// SpawnEvent is one spawn_events row. id is AUTO_INCREMENT (confirmed via SHOW CREATE TABLE) — a
// local surrogate, same untrustworthy-across-databases category as spawngroup.id/lootdrop.id/
// spawn2.id — so source's and sink's events are never paired/matched, only listed independently,
// the same restraint NPCLootComparison already established for lootdrop trees. Its next_minute/
// next_hour/next_day/next_month/next_year columns are continuously rewritten by the server as each
// event reschedules itself, which would make even a same-id diff show "modified" almost constantly
// for otherwise-identical content — one more reason a real diff here wouldn't be meaningful.
type SpawnEvent struct {
    Id     int64
    Fields map[string]interface{}  // spawn_events columns, minus id/zone
}

// ZoneConditionsComparison bundles all three zone-scoped condition-related tables for one zone.
// Comparison-only, no sync/write action for any of the three — the user asked for read-only
// visibility specifically ("just give me read-only on all 3 for now"), sidestepping the
// spawn_condition_values safety question rather than resolving it. Neither table has a version
// column (confirmed via the schema dump) — unlike spawn2/grid, results here don't vary by zone
// version, only by short_name.
...
- `SyncSpawnGroup(options SyncSpawnGroupOptions) (SpawnGroupSyncResult, error)` — dry-run/execute for reconciling one spawngroup (fields + full spawnentry roster together). Identifies the target via a spawn2 location's coordinates, same as everywhere else spawngroup identity is derived. If the sink spawn2 row's `SpawnGroupId` is dangling (`SpawnPoint.SpawnGroupMissing`), this creates a fresh spawngroup instead of updating a nonexistent one, and repoints **every** sink spawn2 row in the zone/version still carrying that same dangling id — not just the one the caller identified — since `SyncSpawnPoints` copies the identical raw source id to every location sharing a spawngroup, so one "sync spawngroup" click resolves the whole group, not one location at a time. Blocked outright (not just warned) if the sink's spawngroupID is referenced by any spawn2 row outside the caller's zone/version, same as before this create-path existed. **Real, shipped bug, fixed 2026-07-26** (user report: "the spawngroup id matters - it needs to actually match the id from the source"): the create-path's `insertSpawnGroupWithNameFallback` call passed no id override, so MySQL auto-assigned whatever id was next in the *sink's own* sequence — almost never `sourcePoint.SpawnGroupId`. But the dangling id being replaced (`sinkPoint.SpawnGroupId`) is itself the raw value `SyncSpawnPoints` already copied verbatim from source — so the id correspondence between source and sink already existed going into this call, and letting auto-increment assign a fresh id then repointing spawn2 onto it *discarded* a correspondence that was already correct, rather than fixing anything. Fixed by forcing an explicit `"id": sourcePoint.SpawnGroupId` override on the insert (mirroring `RelocateSpawnGroup`'s reclaim step, which already did this for the same reason) — the repoint step immediately after is now a no-op in the normal case (the forced id already matches what spawn2 rows were pointing at), and only actually moves anything in the edge case where source's own spawngroup link for this spot changed between when spawn2 was synced and now. If source's id is already occupied by unrelated sink content (a genuine `SpawnGroupCollisionRisk`), this now fails loudly on the `INSERT` rather than silently creating under a mismatched id — that case is meant to be resolved via Relocate & reclaim first. `go build`/`vet`/`test -race` clean; no API/type shape changed, so no `wails generate module` needed.
...
- `fetchSpawnConditions`/`fetchSpawnConditionValues`/`fetchSpawnEvents(ctx, db, zoneShortName string)` (`conditions.go`, added 2026-07-25) — plain `SELECT * FROM <table> WHERE zone = ?` via the shared `scanDynamicRows`, converted to the small structs above using `toInt64`/`withoutFields` — no new low-level helpers needed, everything reused from `dbutil.go`
- `CompareZoneConditions(zoneShortName string) (ZoneConditionsComparison, error)` (`conditions.go`, added 2026-07-25) — fetches all three tables for both sides concurrently via the existing `runParallel`, one goroutine per side doing its own three sequential fetches, mirroring `CompareSpawns`'/`CompareSpawnGroups`' exact shape. Diffs `spawn_conditions` by `Id` (mirrors `CompareGrids`'s match-by-`Id`/`mapsEqual` shape exactly, including `mapsEqual`'s existing `id` exclusion doing double duty here); `SourceValues`/`SinkValues`/`SourceEvents`/`SinkEvents` are collected as-is, never matched — see each type's own comment for why
...
// Conditions tab (useSpawnConditions.js, added 2026-07-25) — read-only visibility for
// spawn_conditions/spawn_condition_values/spawn_events, see conditions.go's own comment for why
// there's no selection/sync state at all here, unlike every other domain hook. Loaded eagerly on
// every zone switch (selectZone's fan-out), same as every other tab's diff data.
...
- **Conditions tab (spawn_conditions / spawn_condition_values / spawn_events), added 2026-07-25 — read-only visibility.** Direct response to "need to add visibility for spawn conditions and spawn_conditions_values and spawn_events (assuming these tables aren't server-specific cache data)." Real schema was pulled via `SHOW CREATE TABLE` on all three before any code was written (this project's standing discipline for a new table), which both corrected and confirmed the user's own stated uncertainty:
  - **The real table name is `spawn_condition_values`** (singular "condition") — not `spawn_conditions_values`, the user's own first guess.
  - **Confirmed: `spawn_condition_values` is live/runtime state, not authored content.** Its schema `(id, value, zone, instance_id)`, unique on `(id, instance_id, zone)`, has `instance_id` as the tell — EQEmu's instanced-zone feature assigns these per running instance, so a value tracked per-instance is inherently ephemeral server state (a boss up/down flag, a quest-stage gate), not something a content dev would push dev→live. Given this, and given the user explicitly said "just give me read-only on all 3 for now," this whole pass is comparison-only — no sync/write action for any of the three tables, sidestepping the safety question entirely rather than resolving it.
  - Each table's own `id`-column trust profile drove a different presentation: `spawn_conditions` (PK `(zone, id)`, `id` NOT auto-increment — zone-scoped and stable, same trust category as `grid.id`) gets a **real diff**, new/modified/removed/match, matched by `Id`. `spawn_condition_values` (no portable identity, see above) is **read-only, informational only**, source/sink shown side by side, never diffed. `spawn_events` (PK `id`, `AUTO_INCREMENT` — a local surrogate, same untrustworthy category as `spawngroup.id`/`lootdrop.id`, plus continuously-server-rewritten `next_minute/hour/day/month/year` scheduling columns that would make even a same-id diff show "modified" almost constantly) is shown as **two independent lists**, never paired — the same restraint `NPCLootComparison` already established for lootdrop trees.
  - Neither table has a `version` column (confirmed via the same schema check) — results are scoped by zone `short_name` only, unlike spawn2/grid.
...
- **`useLoot.js`/`useReferenceDrawer.js` out-of-order-response race, fixed 2026-07-25 (same day, found via user report): "with the loottable sync content from source - I noticed an issue with stale data (it would not update the lootdrops I added to sink). Refresh didn't make it update - I only got it to update after I clicked on the zone name again."** A real, shipped bug, not a caching/DB issue — confirmed by the user's own diagnostic: re-selecting the zone (which forces one clean lookup with nothing else in flight) showed the correct, already-committed data, meaning the write had succeeded and the staleness was purely a frontend display race, not a backend or database problem. Root cause: `useLoot.js`'s `runLookup` (the one shared fetch path behind `lookupLootByNpc`/`lookupLootByRawId`/`refreshWithIds`/`refreshLoot`) had no guard against out-of-order responses. The realistic trigger: after syncing a loottable's content from source, several `loottable_entries` rows can be left pointing at lootdrop ids that don't exist on the sink yet (`Drop: nil`, see `fetchLootDrops`) — clicking "create in sink" for each dangling entry in turn is the natural next step, and each `CreateLootDrop` success already auto-triggers its own `refreshLoot()` call (see `refreshLootAfterCreate`/`refreshLootAfterSyncContent` in `App.jsx`). Clicking through several of these quickly means multiple `CompareNPCLoot` requests can be in flight at once, and nothing guarantees network/response timing preserves the order they were sent in — an earlier request's response landing *after* a later one silently overwrote the fresh `lootComparison` with stale data via a plain `setLootComparison(result)`, and a manual "⟳ Refresh" click was just one more request that could itself lose the same race, which is exactly why it didn't help either. Fixed with the standard pattern: a `requestIdRef` counter incremented on every `runLookup` call, captured locally as `requestId`, checked before applying the response (`if (requestId !== requestIdRef.current) return`) — only the most recently issued request's response is ever allowed to update state, any older one is silently discarded rather than clobbering something newer. `useReferenceDrawer.js`'s `openReferenceComparison` had the identical structural gap (no request-sequencing guard) and is now called from more places than before this session (`refreshReferenceAfterAlign`/`refreshReferenceAfterSyncContent`, both added this session) — fixed the same way pre-emptively, even though it has no equivalent rapid-multi-click workflow making it as likely to actually bite. `go build`/`vet` (no Go changes) and `vite build` clean; frontend-only fix, no `wails generate module` needed.
- **Arrow-key row navigation for diff lists, added 2026-07-25 (same day, direct follow-up).** Direct response to "if I select a row in a list and up or down arrow, it moves the scrollbar... the better experience is to move up or down one row in the list." None of the four diff lists (NPCs, Spawn Points, Grids, Spawngroups) had any `tabIndex`/keyboard handling before this — Up/Down fell through to the browser's default "scroll the nearest scrollable ancestor" behavior. New shared `frontend/src/hooks/useListArrowKeyNav.js` (mirrors `useModalFocusTrap.js`'s "extract once, reuse across every list" shape): takes the caller's own already-filtered-and-sorted `rows` array, a `getKey`/`selectedKey` pair, and an `onSelect` callback; on Up/Down calls `e.preventDefault()` (stopping the default scroll), finds the current row's index, moves to the adjacent one, and calls `onSelect` — clamping at the top/bottom rather than wrapping around, the standard listbox/file-browser/spreadsheet convention, not an invented one. Also scrolls the newly-selected row into view (`scrollIntoView({block: 'nearest'})`, only fires if it's actually off-screen) via a `data-row-key` attribute on each row and a `querySelector` scoped to the list's own container ref, rather than a per-row ref map.
  - Each of the four call sites (`NpcsTab.jsx`/`SpawnsTab.jsx`/`GridsTab.jsx`'s List view/`SpawngroupsTab.jsx`) needed the same small refactor first: the filter/sort chain that builds the rendered list was inline inside the `.map()` call, so it was pulled out into a `visibleRows` variable before the `return` (pure refactor, no rendering change) — arrow nav needs the *exact* same array the user is looking at, not a re-derived one that could drift out of sync with it. The scrollable rows container then gained `ref`/`tabIndex={-1}`/`onKeyDown`/`outline-none` (same no-visible-focus-ring convention modals already use) and `onClick={e => e.currentTarget.focus()}` so any row click also gives the container keyboard focus for the arrow keys to land on — safe to add alongside the existing row `onClick` since only the checkbox's own click stops propagation, not the row's.
  - Deliberately out of scope: Grids' Map-view compact grid picker (a smaller, secondary list with its own row shape), Home/End/Left/Right keys, and the Loot tab's NPC picker (which doesn't currently highlight a "selected" row at all — adding that is a bigger change than adding nav to an existing selection). `vite build` clean (no Go changes); not yet manually smoke-tested in `wails dev`.
- **Conditions tab (spawn_conditions/spawn_condition_values/spawn_events), added 2026-07-25 (same day, direct follow-up).** Direct response to "need to add visibility for spawn conditions and spawn_conditions_values and spawn_events (assuming these tables aren't server-specific cache data)." See the "Conditions tab" bullet under Sync Design for the full design; worth calling out here specifically since it's a good example of this project's "verify, don't guess" discipline paying off in a genuinely new direction: the user's own uncertainty ("assuming these tables aren't server-specific cache data") was exactly right to raise, and pulling the real `SHOW CREATE TABLE` schema before writing anything both caught a naming mistake (the real table is `spawn_condition_values`, singular, not the plural first guess) and settled the safety question outright (`instance_id` in that table's schema is the concrete tell that it's live per-instance server state, not content) — rather than the user having to intuit that from EQEmu internals alone, or worse, this app silently offering to "sync" something that would have reset a live server's actual game progress. The user's own response to that finding — "just give me read-only on all 3 for now" — sidestepped the harder design question (should this ever be writable, and how) entirely, which is exactly the same category of deliberate deferral this app has used before (phase 1 loot/faction/spells/merchant shipped comparison-only long before phase 2 added writes). `go build`/`vet`/`test -race` and `vite build` clean; `wails generate module` run for `CompareZoneConditions` and its new types. Not yet manually smoke-tested in `wails dev`.
- **UI/UX consistency pass, added 2026-07-25 (same day, direct follow-up to a design-review artifact).** Asked for an outside "iPhone-test" read on the interface (not a features audit — "does someone open this cold and know what to do"), which surfaced four findings; the user greenlit implementing three of them directly (the fourth, consolidating Spawn Points/Spawngroups/Grids/Conditions under one "Spawns" tab, stayed a mockup only — the user was explicit they're "not sold" on it yet, so it wasn't touched).
  - **Action-button consistency.** Every "Align ID to source →" / "Sync content from source →" / "Delete spawngroup from sink →" / "Relocate & reclaim spawngroup →" / "Sync spawngroup from source →" trigger was a bare underlined text link — same visual weight as a navigation link, despite opening a confirm-and-write flow exactly like the app's primary yellow sync buttons. All 13 of these (`FactionComparison.jsx`, `SpellsComparison.jsx`, `MerchantComparison.jsx`, `LootTab.jsx`'s three loottable/lootdrop-align triggers, `SpawnDetailPanel.jsx`'s three warning-banner triggers, `SpawnGroupDetailPanel.jsx`'s two) now use a consistent bordered-pill treatment (`px-2 py-1 rounded text-xs border border-<color>-700 text-<color>-400 hover:border-<color>-400 hover:text-<color>-300`), color preserved per action (cyan=rename, amber=content-sync, red=delete/relocate, yellow=final two-step-align confirm) — extending, not inventing, the convention the "Excluded fields" button and the batch-relocate header button already established. Deliberately **not** applied to `LootTab.jsx`'s per-row `AlignTrigger`/"create in sink" links inside the lootdrop tree itself, or `SpawnDetailPanel.jsx`'s "Select all N →" — those are dense-table row actions (or, for "Select all," a non-destructive selection convenience), not the "commit to a write" moment the finding was actually about; turning every row's inline link into a bordered button would add weight to a data-dense tree without the same payoff.
  - **Icon legend, generalized.** The Grids Map view already had a small always-visible legend explaining its marker colors — genuinely good, and never reused elsewhere. New `IconLegend.jsx` (`{items: [{icon, label}]}`) generalizes that exact pattern; wired into `NpcsTab.jsx` (⚡ quest-spawned / ⚠ missing reference / ⊘ excluded-from-sync — previously hover-tooltip-only) and `SpawngroupsTab.jsx` (the bare ⚠ meaning two different things depending on which row it's on — ambiguous-match vs. entries-differ — disambiguated in the legend text since the glyph itself can't carry two meanings, the row's own background tint is the real signal). `SpawnsTab.jsx`/`GridsTab.jsx`/`LootTab.jsx` were deliberately left alone — their own icons/badges already carry inline text labels (e.g. "missing spawngroup", "(source only)"), not bare glyphs, so a legend would explain something already self-explaining.
  - **Persistent header decluttering.** The NPCs-only "Excluded fields" button lived in the shared zone header (visible, doing nothing, on every other tab) purely because that's where it was first added, not because it belonged there. Moved into `NpcsTab.jsx`'s own toolbar row, next to Show All/Differences Only — same "only visible when relevant" result as a conditional, but for free, since `NpcsTab` itself only renders when its tab is active. The shared header now carries only the one primary action (`Sync N NPCs`) for that tab, matching every other tab's header shape.
  - `go build`/`vet` (no Go changes) and `vite build` clean; frontend-only, no `wails generate module` needed. Not yet manually smoke-tested in `wails dev`.
