...
function ConfirmGridSyncModal({showGridSyncConfirm, setShowGridSyncConfirm, dbSinkName, gridSyncPreview, executeGridSync}) {
    const gridSyncConfirmModalRef = useRef(null)
    useEffect(() => {
        if (showGridSyncConfirm) gridSyncConfirmModalRef.current?.focus()
    }, [showGridSyncConfirm])

    if (!showGridSyncConfirm) return null
    return (
        <div
            ref={gridSyncConfirmModalRef}
            tabIndex={-1}
            onKeyDown={e => {
                if (e.key === 'Escape') {
                    e.preventDefault()
            ...
            <div className="bg-gray-800 p-6 rounded-lg w-96 flex flex-col gap-3">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={() => setShowGridSyncConfirm(false)}>✕</button>
                </div>
