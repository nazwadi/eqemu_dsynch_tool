...
// Confirm-before-execute modal for AlignSpawnGroupId — mirrors ConfirmAlignIdModal's shape (the
// generic AlignId primitive's own modal), summary-level rather than a per-entry table, since a
// spawngroup rename touches spawnentry/spawn2 rows counted, not individually listed.
function ConfirmAlignSpawnGroupIdModal({
    showAlignConfirm, setShowAlignConfirm,
    alignError, alignPreview,
        ...
        <div
            ...
            <div className="bg-gray-800 p-6 rounded-lg w-96 flex flex-col gap-3">
                <div className="flex justify-between items-center mb-2">
                    <h2 className="text-lg font-medium">Align Spawngroup ID</h2>
                ...
                {alignError ? (
                    ...
                    <>
                        ...
                        <div className="text-sm text-gray-300">
                            Sink's spawngroup #{alignPreview.RenamedFrom} will be renamed to #{alignPreview.RenamedTo}, matching source — its own fields/spawn entries are untouched, only the id changes.
                        </div>
                        {alignPreview.SquatterEvicted && (
                            <div className="text-sm text-cyan-400">
                                #{alignPreview.RenamedTo} is currently occupied by "{alignPreview.SquatterSummary}" — that spawngroup (and its own spawn entries) will be moved to a new id first, so nothing is lost.
                            </div>
                        )}
                        <div className="text-sm text-gray-300">
                            {alignPreview.SpawnPointsRepointed} spawn2 location{alignPreview.SpawnPointsRepointed === 1 ? '' : 's'} — across every zone that references #{alignPreview.RenamedFrom} — will be repointed to #{alignPreview.RenamedTo}.
                        </div>
...
export default ConfirmAlignSpawnGroupIdModal
